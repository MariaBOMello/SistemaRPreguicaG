...
namespace SistemaRPreguicaG
{
    public partial class FrmVizualizarPersonagens : Form
    {
        ...
        public FrmVizualizarPersonagens(int idCampanha)
        {
            ...
            this.Text = $"Personagens da Campanha";
            CarregarPersonagens();
        }
        ...
        private void BtnInativarPersonagem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                int idPersonagem = Convert.ToInt32(DgvPersonagens.CurrentRow.Cells[0].Value);
                string nomePersonagem = DgvPersonagens.CurrentRow.Cells[1].Value.ToString();

                var resultado = MessageBox.Show($"Inativar o personagem '{nomePersonagem}'?",
                ...
                if (resultado == DialogResult.Yes)
                {
                    using (SqlConnection con = new SqlConnection(connectionString))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, con))
                        {
                            ...
                            MessageBox.Show("✅ Personagem inativado!", "Sucesso",
        ...
        {
            // COMENTAR todo este método se existir
        }
    }
}